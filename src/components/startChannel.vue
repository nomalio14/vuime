      ...
      <template v-slot:items="props">
        <tr @click="clickContact(props.item.userId)">
          <td class="userName">
            ...
            <td>
            <v-icon v-if=props.item.selected dark color="teal lighten-3">check</v-icon>
          </td>
        </tr>
      </template>
...
export default {
  ...
  methods: {
    clickContact(id) {
      ...
      this.listedContacts[clickedIndex].selected = !this.listedContacts[clickedIndex].selected
    },
    createCancel() {
      this.dialog = false
    },
    ...
    openMakeChannel() {
      this.listedContacts = this.contacts.map(
        contact => {
          return {
            selected:false,
            ...contact
